...
df <- read.table(data_path, header = FALSE, sep = "", stringsAsFactors = FALSE)
colnames(df) <- gwas_cols
head(df)
...
```{r raw 100k bonferroni, echo = TRUE, message = FALSE, warning = FALSE}
df3_clean <- df3 %>%
  filter(!is.na(P))
...
```{r chromosome 6 raw pvalues, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r BKY plot test, echo = TRUE, message = FALSE, warning = FALSE}
plot_adjusted_pvalues(
  df3_clean,
  pval_col = "P_BKY",
  plot_title = "BKY-adjusted p-values across the genome",
